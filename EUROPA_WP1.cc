...
/**
 * @brief Main function of the EUROPA_WP1 program.
 * @param argc Number of command-line arguments
 * @param argv Array of command-line arguments
 * @return Exit code (0 = success, 1 = failure)
 *
 * This program can run in two modes:
 * 1. Visualization mode: `./EUROPA_WP1 outputFile`
 * 2. Batch mode: `./EUROPA_WP1 outputFile NParticles macro MT ON/OFF [threads]`
 */
int main(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        G4Exception("Main", "main0004", FatalException,
                    "Insufficient input arguments. Usage: ./EUROPA_WP1 [ROOT file name] [events] [macro] [MT ON/OFF] [threads (if MT ON)]");
        return 1;
    }
    ...
    EUROPA_WP1GeometryConstruction *GeomCons = new EUROPA_WP1GeometryConstruction;
    runManager->SetUserInitialization(GeomCons);

    /** Initialize physics */
    runManager->SetUserInitialization(new EUROPA_WP1Physics);

    /** Initialize user actions */
    runManager->SetUserInitialization(new EUROPA_WP1ActionInitialization(
